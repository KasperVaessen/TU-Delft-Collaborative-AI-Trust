...
class BaseAgent(BaseLineAgent):
    ...
    def _processMessages(self, teamMembers, state):
        ...
        for member in messages.keys():
            ...
            for msg in msgs:
                if 'Moving to ' in msg:
                    room_id = msg.split(' ')[-1]
                    current_world_state['teammembers'][member]['state'] = {'type': Status.MOVING_TO_ROOM,
                                                                           'room_id': room_id}
                ...
                if 'Searching through ' in msg:
                    ...
                    for block in current_world_state['found_blocks']:
                        if block_vis == block['visualization'] and block_loc == block['location']:
                            ...
                            break
                    for goal in [g for g in current_world_state['goals'] if
                                 g['visualization'] == block_vis and g['location'] == block_loc]:
            ...
            for message in received[member]:
                if 'Dropped' in message:
                    ...
                    for goal in self._world_state['goals']:
                        if goal['visualization'] == block_vis and not block_loc == goal['location'] and not goal[
                            'satisfied'] and not self.previousGoalsSatisfied(goal['index'] - 1):
    ...
    def _processObservations(self, state):
        ...
        blocks = copy.deepcopy([block for block in state.values()
                                if 'class_inheritance' in block and 'CollectableBlock' in block['class_inheritance'] and
                                block['is_collectable']])
        for block in blocks:
            block_vis = block['visualization']
            block_vis.pop('opacity')
            block_vis.pop('visualize_from_center')
            block_vis.pop('depth')
            observations['blocks'].append(
                {'obj_id': block['obj_id'], 'visualization': block_vis, 'location': block['location']})
        ...
        for door in doors:
            observations['doors'].append(
                {'room_name': door['room_name'], 'is_open': door['is_open'], 'obj_id': door['obj_id']})
    ...
    def _handleObservations(self, observations):
        ...
        for block in observations['blocks']:
            # find possible goal positions of block
            goals = [g for g in self._world_state['goals']
                     if g['visualization'] == block['visualization']]

            if len(goals) != 0:
                # this block does not have a goal, so no messaging is required
                self._sendMessage(
                    'Found goal block {} at location {}'.format(json.dumps(block['visualization']), block['location']),
                    self._you['obj_id'])

            exists = False
            # only add a block if it is not in the current world state (duplicates are not really usefull in the world_state
            for b in self._world_state['found_blocks']:
                if b['visualization'] == block['visualization'] and b['location'] == block['location']:
                    exists = True
                    break
            if not exists:
                self._world_state['found_blocks'].append(
                    {'visualization': block['visualization'], 'location': block['location'], 'by': self._you['obj_id']})
        ...
        # handle goal verification (done for goals in a radius of 1 manhattan distance)
        goals_to_verify = [goal for goal in self._world_state['goals']
                           if not goal['verified']
                           and self.dist(self._you['location'], goal['location']) <= 1]
    ...
    def decide_on_bw4t_action(self, state: State):
        ...
        while True:
            ...
            if Phase.FOLLOW_PATH_TO_BLOCK == self._phase:
                ...
                if action is not None:
                    ...
                    try:
                        ...
                        targets_in_range = list(filter(lambda b: b['visualization'] == closest_target['visualization'],
                                                       observations['blocks']))
            ...
            if Phase.SEARCH_ROOM == self._phase:
                ...
                self._world_state['searched_rooms'].append(
                    {'room_id': self._current_room['room_name'], 'by': agent_name})
                self._phase = Phase.PLAN_NEXT_ACTION
            ...
            if Phase.PLAN_FIX_SOLUTION == self._phase:
                missing_goals = sorted(self.get_missing_goals(), key=lambda g: g['index'])
                first_unsat_goal_index = 0
                if len(missing_goals) > 0:
                    first_unsat_goal_index = missing_goals[0]['index']
                goals = [goal for goal in self._world_state['goals']
                         if goal['index'] >= first_unsat_goal_index]
                self._navigator.reset_full()
                for goal in goals:
                    self._navigator.add_waypoint(goal['location'])
                    self._phase = Phase.FIX_SOLUTION
                    self._current_state = {'type': Status.FIXING_SOLUTION, 'cur_goal': first_unsat_goal_index}
    ...
    def plan_next_action(self, state):
        ...
        if (len(state.get_self()['is_carrying']) < self._carrying_capacity
            ...
            block_vis = copy.deepcopy(state.get_self()['is_carrying'][0]['visualization'])
            block_vis.pop('visualize_from_center')
            block_vis.pop('opacity')
            block_vis.pop('depth')

            sat_goals = sorted([goal for goal in self._world_state['goals'] if goal['satisfied']],
            ...
            if len([goal for goal in self._world_state['goals'] if not goal['verified']]) > 0:
                ...
                # after possibly verifying the goals there exists goals that are prematurely satisfied, fix those
                self.plan_drop_beside_goal(state)
                self.__next_phase.append(Phase.PLAN_FIX_SOLUTION)
            elif block_vis != missing_goals[0]['visualization']:
                # we are carrying a goal block that should not yet be satisfied (probably due to a lie or miscommunication)
                # drop this block next to its goal and make a new plan
                self.plan_drop_beside_goal(state)
            else:
    ...
    def plan_path_to_block(self, state):
        ...
        if possible_target is None:
            ...
            self._current_state = {'type': Status.PICKING_UP_BLOCK,
                                   'block': {'visualization': possible_target['visualization'],
                                             'location': possible_target['location']}}
    ...
    def pickup_block(self, agent_name, observations, block_id):
        ...
        for block in observations['blocks']:
            if block['obj_id'] == block_id:
                ...
                goals_with_same_vis = [g for g in self._world_state['goals'] if
                                       g['visualization'] == block['visualization']]
    ...
    def drop_beside_goal(self, state):
        ...
        self._sendMessage('Dropped goal block {} at drop location {}'.format(json.dumps(block_to_drop['visualization']),
                                                                             state.get_self()['location']),
                          state.get_self()['obj_id'])
        self._phase = Phase.PLAN_NEXT_ACTION
        return DropObject.__name__, {'object_id': block_to_drop['obj_id']}
    ...
    def init_goals(self, state):
        blocks = copy.deepcopy(
            [{'visualization': tile['visualization'], 'location': tile['location']} for tile in state.values()
        ...
        for i in range(len(blocks)):
            ...
            blocks[i][
                'verified'] = True  # at the start of a game all goals are standard not satisfied, so we can assume this is verified
            blocks[i]['by'] = None
